...
module(
  'Integration | Component | standard/core/setup-routes',
  function (hooks) {
    ...
    test('it renders', async function (assert) {
      ...
      await render(
        hbs`<Standard::Core::SetupRoutes @recordWeb={{this.recordWeb}} @serviceAvailable={{this.serviceAvailable}} />`
      );

      assert.dom('#setup-routes-info').exists('Accordion info esiste');
      ...
      assert.dom('button.btn-save-field').exists('bottone aggiungi esiste!');
    });
  }
);
