...
module('Integration | Component | standard/setup-aspect', function (hooks) {
  ...
  test('it renders', async function (assert) {
    ...
    await render(
      hbs`<Standard::Core::SetupAspect @recordWeb={{this.recordWeb}} @serviceAvailable={{this.serviceAvailable}}/>`
    );
    ...
    let divColore = 0;
    $('.theme-settings-swatches').each(function () {
      divColore++;
    });

    console.log(divColore);

    assert
      .dom('.theme-settings-swatches')
      .exists('Il div per la scelta dei colori esiste, quantità: ' + divColore);
