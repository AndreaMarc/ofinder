...
{
    /// <summary>
    /// Analyzes SQL Server database schema.
    /// </summary>
    public class SqlServerAnalyzer : IDatabaseAnalyzer
    {
        ...
        public string GetDatabaseName(string connectionString)
        {
            try
            {
                ...
                return builder.InitialCatalog ?? "Database";
            }
            catch
            {
                return "Database";
        ...
        public async Task<List<TableSchema>> AnalyzeSchemaAsync(string connectionString)
        {
            ...
            foreach (var (schema, tableName) in tableNames)
            {
                ...
                table.Columns = await GetColumnsAsync(connection, schema, tableName, table.EntityName);

                tables.Add(table);
            }

            // Get relationships (foreign keys)
            await PopulateRelationshipsAsync(connection, tables);
        ...
        private async Task<List<ColumnSchema>> GetColumnsAsync(SqlConnection connection, string schema, string tableName, string entityName)
        {
            ...
            while (await reader.ReadAsync())
            {
                ...
                var column = new ColumnSchema
                {
                    ...
                    DataType = dataType,
                    CSharpType = SqlTypeMappings.MapToCSharp(dataType, DatabaseEngine.SqlServer),
        ...
        private async Task PopulateRelationshipsAsync(SqlConnection connection, List<TableSchema> tables)
        {
            string query = @"
                SELECT
                    fk.TABLE_SCHEMA,
            ...
            while (await reader.ReadAsync())
            {
                ...
                string referencedSchema = reader.GetString(3);
                string referencedTable = reader.GetString(4);
                string referencedColumn = reader.GetString(5);

                // Find source table
                var sourceTable = tables.FirstOrDefault(t =>
                    t.Schema == tableSchema && t.TableName == tableName);
                ...
                var referencedTableObj = tables.FirstOrDefault(t =>
                    t.Schema == referencedSchema && t.TableName == referencedTable);
